...
CORE_CLASSES = [
    ...
    core_nodes.GamutNormalDecode, core_nodes.GamutNormalEncode,
    io_nodes.GamutLoadEXR, io_nodes.GamutSaveEXR, io_nodes.GamutSaveDataEXR,
    io_nodes.GamutSaveHighBit, io_nodes.GamutSaveMultilayerEXR, io_nodes.GamutLoadDataEXR,
...
# Nodes on the data path: no colour declaration, by design.
DATA_CLASSES = frozenset({
    core_nodes.GamutNormalDecode, core_nodes.GamutNormalEncode, io_nodes.GamutSaveDataEXR,
    io_nodes.GamutLoadDataEXR,
})
...
EXPECTED_RETURNS = {
    ...
    "GamutLoadDataEXR": ("IMAGE",), "GamutSaveMultilayerEXR": (),
    "GamutNormalDecode": ("IMAGE",), "GamutNormalEncode": ("IMAGE",),
    "GamutSaveEXR": (), "GamutSaveDataEXR": (), "GamutSaveHighBit": (),
...
def test_every_node_has_a_consistent_comfyui_contract(node):
    ...
    # Colour nodes carry an optional COLORSPACE socket; the data nodes must NOT.
    # A surface normal has no primaries and no transfer curve, so offering it a
    # colour declaration would invite exactly the mislabelling the data path
    # exists to prevent. The absence is asserted, not merely tolerated.
    if node in DATA_CLASSES:
        assert "colorspace" not in schema.get("optional", {})
        assert "colorspace" not in schema["required"]
    else:
...
spec.loader.exec_module(module)
assert len(module.NODE_CLASS_MAPPINGS) == {core_count}
...
"""
    result = subprocess.run(
        [sys.executable, "-c", script.format(core_count=len(CORE_CLASSES))], cwd=ROOT,
...
# --- queue-time overwrite preflight ------------------------------------------


_SAVE_NODES = [
    (io_nodes.GamutSaveEXR, "exr", {}),
    (io_nodes.GamutSaveMultilayerEXR, "exr", {}),
    (io_nodes.GamutSaveDataEXR, "exr", {}),
    (io_nodes.GamutSaveHighBit, "png", {"format": "png16"}),
]


@pytest.mark.parametrize("node, extension, extra", _SAVE_NODES)
def test_queue_time_preflight_refuses_an_existing_first_frame(tmp_path, node, extension, extra):
    """A forty-minute render must not discover the collision at its last node."""
    existing = tmp_path / f"shot_v001.1001.{extension}"
    existing.write_bytes(b"")
    verdict = node.VALIDATE_INPUTS(prefix=str(tmp_path / "shot"), **extra)
    assert verdict is not True
    assert str(existing) in verdict and "version" in verdict


@pytest.mark.parametrize("node, extension, extra", _SAVE_NODES)
def test_queue_time_preflight_passes_a_free_path(tmp_path, node, extension, extra):
    assert node.VALIDATE_INPUTS(prefix=str(tmp_path / "shot"), **extra) is True


def test_queue_time_preflight_honours_frame_and_version_widgets(tmp_path):
    (tmp_path / "shot_v012.000007.exr").write_bytes(b"")
    refused = io_nodes.GamutSaveEXR.VALIDATE_INPUTS(
        prefix=str(tmp_path / "shot"), start_frame=7, frame_pad=6, version=12,
    )
    assert refused is not True
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(
        prefix=str(tmp_path / "shot"), start_frame=7, frame_pad=6, version=13,
    ) is True


@pytest.mark.parametrize("prefix", [None, "", "shots/%width%x%height%/beauty"])
def test_queue_time_preflight_passes_through_what_it_cannot_resolve(prefix):
    """A linked prefix arrives as None before execution; a template needs the image."""
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix=prefix) is True


def test_queue_time_preflight_leaves_argument_errors_to_the_writer(tmp_path):
    """save() names a bad frame_pad itself; the preflight must not pre-empt that message."""
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix=str(tmp_path / "shot"), frame_pad=0) is True


def test_queue_time_preflight_resolves_relative_prefixes_through_comfyui(monkeypatch, tmp_path):
    def save_path(prefix, output_dir, width, height):
        return str(tmp_path / "shots"), "beauty", 1, "shots", prefix

    monkeypatch.setattr(io_nodes, "folder_paths", SimpleNamespace(
        get_output_directory=lambda: str(tmp_path), get_save_image_path=save_path,
    ))
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix="shots/beauty") is True
    (tmp_path / "shots").mkdir()
    (tmp_path / "shots" / "beauty_v001.1001.exr").write_bytes(b"")
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix="shots/beauty") is not True


def test_queue_time_preflight_reports_comfyui_path_refusals(monkeypatch):
    def escape(prefix, output_dir, width, height):
        raise Exception("Saving image outside the output folder is not allowed.")

    monkeypatch.setattr(io_nodes, "folder_paths", SimpleNamespace(
        get_output_directory=lambda: "out", get_save_image_path=escape,
    ))
    verdict = io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix="../escape")
    assert "outside the output folder" in verdict


def test_queue_time_preflight_is_silent_outside_comfyui(monkeypatch):
    monkeypatch.setattr(io_nodes, "folder_paths", None)
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix="relative/shot") is True


def test_write_time_paths_and_preflight_paths_agree(tmp_path):
    """One path builder for both, or the preflight can pass a path the writer then refuses."""
    folder, name = io_nodes._output_target(str(tmp_path / "shot"))
    first = io_nodes._frame_path(folder, name, "exr", 1001, 4, 1)
    io_nodes.GamutSaveEXR().save(torch.ones((1, 2, 3, 3)), str(tmp_path / "shot"))
    assert first.exists()
    assert io_nodes.GamutSaveEXR.VALIDATE_INPUTS(prefix=str(tmp_path / "shot")) is not True


def test_existing_io_controls_and_highbit_schema_are_preserved():
    """Saved workflow widgets serialize in this order; defaults must not drift."""
    expected = {
        io_nodes.GamutLoadEXR: [("path", ""), ("fallback_primaries", "rec709"),
                              ("fallback_transfer", "linear"), ("tonemap_preview", False)],
        io_nodes.GamutSaveEXR: [("image", None), ("prefix", "Gamut/render"), ("primaries", "rec709"),
                              ("transfer", "linear"), ("half", True), ("compression", "zip"),
                              ("start_frame", 1001), ("frame_pad", 4), ("version", 1)],
        io_nodes.GamutSaveDataEXR: [("image", None), ("prefix", "Gamut/data"), ("channel_naming", "normals"),
                                  ("channel_prefix", "N"), ("half", True), ("compression", "zip"),
                                  ("start_frame", 1001), ("frame_pad", 4), ("version", 1)],
        io_nodes.GamutSaveHighBit: [("image", None), ("prefix", "Gamut/image"), ("format", "png16"),
                                  ("transfer", "srgb"), ("dither", False)],
    }
    for node, controls in expected.items():
        actual = [(name, field[1].get("default") if len(field) > 1 else None)
                  for name, field in node.INPUT_TYPES()["required"].items()]
        assert actual == controls
    assert io_nodes.GamutSaveHighBit.INPUT_TYPES()["optional"] == {"colorspace": ("COLORSPACE",)}
    for node in (io_nodes.GamutSaveEXR, io_nodes.GamutSaveDataEXR, io_nodes.GamutSaveMultilayerEXR):
        assert not hasattr(node, "IS_CHANGED")


def test_save_exr_embeds_image_alpha_in_float_without_converting_rgb(tmp_path):
    image = torch.tensor([[[[-1., 2., 4.]]]], dtype=torch.float16)
    opacity = torch.tensor([[[[0.1234567]]]], dtype=torch.float32)
    target = tmp_path / "exact.exr"
    io_nodes.GamutSaveEXR().save(image, str(target), half=False, path_mode="filename", alpha_image=opacity)
    pixels, _, _ = exr.read_exr(target)
    np.testing.assert_array_equal(pixels[..., :3], image.numpy()[0])
    assert pixels[0, 0, 3] == opacity[0, 0, 0, 0].item()


@pytest.mark.parametrize("node", [io_nodes.GamutSaveEXR, io_nodes.GamutSaveMultilayerEXR])
def test_late_external_alpha_error_does_not_create_output_directory(tmp_path, node):
    opacity = torch.ones(2, 1, 1, 1)
    opacity[1] = float("nan")
    with pytest.raises(ValueError, match="finite opacity"):
        node().save(torch.ones(2, 1, 1, 3), str(tmp_path / "missing/shot"), alpha_image=opacity)
    assert not list(tmp_path.iterdir())


def test_io_failure_preserves_earlier_complete_frames(tmp_path, monkeypatch):
    writer = io_nodes.exr_layers.write_multilayer_exr
    def fail_second(path, pixels, **kwargs):
        if path.name.endswith("1002.exr"):
            path.write_bytes(b"partial second frame")
            raise OSError("disk error")
        writer(path, pixels, **kwargs)
    monkeypatch.setattr(io_nodes.exr_layers, "write_multilayer_exr", fail_second)
    with pytest.raises(OSError, match="disk error"):
        io_nodes.GamutSaveMultilayerEXR().save(torch.ones(2, 1, 1, 3), str(tmp_path / "shot"))
    first = tmp_path / "shot_v001.1001.exr"
    assert list(tmp_path.iterdir()) == [first]
    assert exr.read_exr(first)[0].shape == (1, 1, 3)


@pytest.mark.parametrize("components", [None, "RGBA", "automatic"])
def test_invalid_data_component_choice_cannot_enter_the_colour_reader(components):
    with pytest.raises(ValueError, match="Data components"):
        io_nodes.GamutLoadDataEXR().load("unused.exr", components=components)
