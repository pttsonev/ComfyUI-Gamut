...
class GamutColorSpaceInfo:
    ...
    def inspect(self, image=None, path="", colorspace=None):
        ...
        if path:
            ...
            header = exr.inspect_exr(resolved)
            cs = exr._infer_colorspace(header)
            ...
            types = exr.channel_types(resolved)
            for layer, channels in exr.channel_groups(header).items():
                description = ", ".join(f"{name} ({types[channel.name]})" for name, channel in channels.items())
                lines.append(f"Layer {layer or '(root)'}: {description}")
            if "gamut:layerRoles" in header:
                lines.append(f"gamut:layerRoles: {header['gamut:layerRoles']}")
